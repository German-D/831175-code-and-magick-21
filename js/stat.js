...
window.renderStatistics = function (ctx, players, times) {
  ...
  let maxTime = times.reduce(function (acc, cur) {
    if (cur > acc) {
      return cur;
    } else {
      return acc;
  ...
  for (let i = 0; i < players.length; i++) {
    ...
    let barHeight = 150 * times[i] / maxTime;
    let barColor = getRandomNumber(1, 100) + `%`;
    let barRemains = MAX_BAR_HEIGHT - barHeight;
    let barX = CLOUD_X + GISTOGRAM_GAP;
    let finalBarGap = GISTOGRAM_GAP + BAR_WEIGHT;
    let finalResultY = RESULT_Y + barRemains;
    let finalBarY = BAR_Y + barRemains;
